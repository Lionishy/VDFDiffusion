...
namespace iki {	namespace math { namespace device {
	template <typename T> inline
		T diagonal_discretization(T fim1, T fi, T fip1, T dfc_left, T dfc_right) {
		return dfc_left * fim1 + dfc_right * fip1 - (dfc_left + dfc_right) * fi;
	}

	template <> inline
		float diagonal_discretization<float>(float fim1, float fi, float fip1, float dfc_left, float dfc_right) {
