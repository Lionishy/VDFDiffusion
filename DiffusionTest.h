...
namespace iki {	namespace diffusion {
	...
	template <typename T>
	void x_y_sin_sin_mixed_term_test(std::vector<T> &f, std::vector<T> &x_dfc, std::vector<T> &y_dfc, std::vector<T> &xy_dfc, std::vector<T> &yx_dfc, size_t x_size, size_t y_size, int Nx, int Ny) {
		{
			auto const PI = T(3.14159265358979323);
			for (size_t y_idx = 0; y_idx != y_size; ++y_idx)
				for (size_t x_idx = 0; x_idx != x_size; ++x_idx)
